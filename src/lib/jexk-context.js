...
const keys = [
  ["jexk-games", (buf) => { gameThumbBuffer = buf; }],
  ["jexk-rpg", (buf) => { rpgThumbBuffer = buf; }],
  ["jexk-winner", (buf) => { winnerThumbBuffer = buf; }],
];
...
function getRpgContextInfo(title, body) {
  ...
  if (title || body) {
    base.externalAdReply = {
      title: title || config.bot?.name || "jexk RPG",
      body: body || "",
...
function createFakeQuoted(botName = "jexk-AI", verified = true) {
  return {
    key: {
      fromMe: false,
