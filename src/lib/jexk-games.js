import {
  ...
  getRandomReward,
  getProgressiveHint,
} from "./jexk-game-data.js";
import { getDatabase } from "./jexk-database.js";
import { addExpWithLevelCheck } from "./jexk-level.js";
import {
  getGameContextInfo,
  sendGamePreview,
  checkFastAnswer,
} from "./jexk-context.js";
import config from "../../config.js";
import fs from "fs";
import sharp from "sharp";
let fetchBuffer;
try {
  fetchBuffer = (await import("./jexk-utils.js")).fetchBuffer;
} catch { }
        ...
        if (session && session.gameType === gameType) {
          ...
          await sock.sendPreview(
            m.chat,
            {
              ...
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["jexk2"])).resize(300, 300).toBuffer(),
              previewType: 0,
            },
          );
          return;
...
const games = new jexkGames();

export { jexkGames, games };
