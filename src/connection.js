...
import * as colors from "./lib/jexk-logger.js";
import { extendSocket } from "./lib/jexk-socket.js";
import {
  isLid,
  lidToJid,
  decodeAndNormalize,
...
import { initAutoBackup } from "./lib/jexk-auto-backup.js";
...
async function startConnection(options = {}) {
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    try {
      ...
      const code = await sock.requestPairingCode(phoneNumber, "ABCDEFGH");
      console.log("");
      console.log(
        colors.createBanner(
          [
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      colors.logger.info(
        "bot",
        `${config.bot?.name || "jexk-AI"} (${n || "?"}) · WA v${version.join(".")}`,
      );

      setTimeout(async () => {
        try {
          const { reloadAllPlugins: R, getPluginCount: G } =
            await import("./lib/jexk-plugins.js");
          !G() && (await R());
        } catch {}
      }, 100);
      ...
      try {
        ...
        const db = (await import("./lib/jexk-database.js")).getDatabase();
  ...
  sock.ev.on("group-participants.update", async (event) => {
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          const { getDatabase } = await import("./lib/jexk-database.js");
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (isStatusMention || hasGroupMentionInContext) {
        ...
        try {
          ...
          const { handleAntiTagSW, handleAntiSwGc } =
            await import("./lib/jexk-group-protection.js");
          const db = getDatabase();
      ...
      if (isOwner && messageBody.startsWith("=>")) {
        ...
        if (code) {
          try {
            const { serialize } = await import("./lib/jexk-serialize.js");
            const m = await serialize(currentSock, msg, {});
            const { getDatabase: _getDb } =
              await import("./lib/jexk-database.js");
            const db = _getDb();
  ...
  {
    const { getDatabase: _getDb } = await import("./lib/jexk-database.js");
    const _db = _getDb();
