...
import { serialize, getCachedThumb } from "./lib/jexk-serialize.js";
import { saluranCtx } from "./lib/jexk-context.js";
import {
  getPlugin,
  getPluginCount,
  getAllPlugins,
  pluginStore,
  getAllCommandNames,
} from "./lib/jexk-plugins.js";
import {
  findSimilarCommands,
  formatSuggestionMessage,
} from "./lib/jexk-similarity.js";
import { getDatabase } from "./lib/jexk-database.js";
import {
  formatUptime,
  createWaitMessage,
  createErrorMessage,
} from "./lib/jexk-formatter.js";
import { getUptime } from "./connection.js";
import { logger, logMessage, c } from "./lib/jexk-logger.js";
import {
  isLid,
  isLidConverted,
  lidToJid,
  ...
  savePersistentCache,
  getLidCacheSize,
} from "./lib/jexk-lid.js";
import { hasActiveSession, getSession } from "./lib/jexk-game-data.js";
import {
  levenshtein,
  formatAfkDuration,
  checkPermission,
  checkMode,
} from "./lib/jexk-middleware.js";
import {
  handleAntilink,
  handleAntiJudol,
  handleAntiPhising,
  ...
  handleAntiHidetag,
  handleAntiSwGc,
} from "./lib/jexk-group-protection.js";
import {
  debounceMessage,
  getCachedUser,
  getCachedGroup,
  getCachedSetting,
} from "./lib/jexk-performance.js";
import {
  isJadibotOwner,
  isJadibotPremium,
  loadJadibotDb,
} from "./lib/jexk-jadibot-database.js";
import { getActiveJadibots } from "./lib/jexk-jadibot-manager.js";
import { handleCommand as handleCaseCommand } from "../case/jexk.js";
import { RateLimiterMemory } from "rate-limiter-flexible";
import { games as jexkGames } from "./lib/jexk-games.js";
import fs from "fs";
import path from "path";
import { exec } from "child_process";
import axios from "axios";
import * as timeHelper from "./lib/jexk-time.js";
const safe = (fn) => {
  try {
    return fn();
  } catch {
...
try {
  levelHelper = await import("./lib/jexk-level.js");
} catch { }
try {
...
try {
  handleAutoAI = (await import("./lib/jexk-auto-ai.js")).handleAutoAI;
} catch { }
try {
  handleAutoDownload = (await import("./lib/jexk-auto-download.js"))
    .handleAutoDownload;
} catch { }
try {
  checkStickerCommand = (await import("./lib/jexk-sticker-command.js"))
    .checkStickerCommand;
} catch { }
try {
  handleStickerReply = (await import("./lib/jexk-sticker-reply.js"))
    .handleStickerReply;
} catch { }
try {
...
async function handleSmartTriggers(m, sock, db) {
  ...
  try {
    ...
    const saluranName = config.saluran?.name || config.bot?.name || "jexk-AI";
    const botName = config.bot?.name || "jexk-AI";
          ...
          if (buffer && buffer.length > 1000) {
            ...
            if (transcript) {
              ...
              if (bestMatch) {
                ...
                const { parseCommand } =
                  await import("./lib/jexk-serialize.js");
    ...
    if (m.body?.startsWith(">>") && m.isOwner) {
      ...
      try {
        ...
        const execCode = new AsyncFunction(
          ...
          const { generateWAMessage, getBuffer, generateWAMessageFromContent, proto, generateMessageID } = await import('jexk')
          const { exec: childExec } = await import('child_process')
          const { VERSION, Button, ButtonV2, Carousel, AIRich, } = await import('./lib/jexk-builder.js')
      ...
      if (checkStickerCommand) {
        try {
          ...
          if (stickerCmd) {
            ...
            const { parseCommand } = await import("./lib/jexk-serialize.js");
            const parsed = parseCommand(m.body, prefix);
            m.isCommand = parsed.isCommand;
            m.command = parsed.command;
            m.args = parsed.args;
    ...
    if (!plugin) {
      ...
      if (similarityEnabled) {
        ...
        if (suggestions.length > 0) {
          ...
          try {
            const { prepareWAMessageMedia } = await import("jexk");
            const media = await prepareWAMessageMedia(
              { image: fs.readFileSync(config.assets["jexk"]) },
...
async function groupHandler(update, sock) {
  try {
    ...
    for (let participant of participants) {
      ...
      const saluranId = config.saluran?.id || "120363400911374213@newsletter";
      const saluranName =
        config.saluran?.name || config.bot?.name || "jexk-AI";
      ...
      const rankActions = {
        promote: {
          ...
          imgKey: "_promoteImg",
          imgPath: config.assets["jexk-promote"],
          emoji: "🎉",
          label: "PROMOTE",
          text: (p, a) =>
        ...
        demote: {
          ...
          imgKey: "_demoteImg",
          imgPath: config.assets["jexk-demote"],
          emoji: "📉",
          label: "DEMOTE",
          text: (p, a) =>
            ...
            viewOnceMessage: {
              message: {
                ...
                interactiveMessage: {
                  ...
                  footer: { text: config.bot?.name || "jexk-AI" },
                  ...
                  contextInfo: {
                    ...
                    forwardedNewsletterMessageInfo: {
                      ...
                      newsletterName: config.saluran?.name || config.bot?.name || "jexk-AI",
                      serverMessageId: 127,
                    },
                  },
                  nativeFlowMessage: {
                    messageParamsJson: JSON.stringify({
                      ...
                      tap_target_configuration: { title: " X ", description: "bomboclard", canonical_url: "https://jexk.site", domain: "shop.example.com", button_index: 0 },
                    }),
